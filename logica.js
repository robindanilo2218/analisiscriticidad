...
const DB_NAME = "MotorIndustrialDB_V7"; const STORE_NAME = "tablas_maestras";
...
// --- REPARADO: ESTANDARIZAR NOMBRES Y CATEGORIZAR LÍNEAS (TRIM COMPLETO) ---
...
function guardarUnificacion() { 
    document.querySelectorAll('input[id^="uniLine_"]').forEach(input => { let crudo = input.getAttribute('data-crudo'); let estandar = input.value.trim(); if (estandar && estandar !== crudo) diccionarioNombres[crudo] = estandar; }); 
    guardarEstado('diccionario_nombres', diccionarioNombres); 
    [baseDeDatos, baseDeCostos].forEach(baseObj => { 
        if(baseObj.length === 0) return; 
        let columnas = Array.from(new Set(baseObj.flatMap(Object.keys))); 
        columnas.forEach(col => { 
            baseObj.forEach(fila => { 
                let v = fila[col]; 
                if (v) { let vTrim = String(v).trim(); if (diccionarioNombres[vTrim]) fila[col] = diccionarioNombres[vTrim]; } 
            }); 
        }); 
    }); 
    guardarEstado('bd_industrial', baseDeDatos); guardarEstado('bd_costos', baseDeCostos); 
    document.getElementById('modalUnificar').close(); actualizarSelectoresGlobales(); actualizarSelectoresCostos(); actualizarListasFiltrosSecundarios(); aplicarFiltro(); alert("¡Nombres estandarizados!"); 
}
...
function guardarCategorizacion() { 
    document.querySelectorAll('input[id^="catLine_"]').forEach(input => { 
        let m = input.getAttribute('data-maquina'); let l = input.value.trim(); 
        if (l) diccionarioLineas[m] = l; else delete diccionarioLineas[m]; 
    }); 
    guardarEstado('diccionario_lineas', diccionarioLineas); 
    [baseDeDatos, baseDeCostos].forEach(baseObj => { 
        if(baseObj.length === 0) return; 
        let columnas = Array.from(new Set(baseObj.flatMap(Object.keys))); 
        let colMaq = columnas.find(c => c.toLowerCase().includes('maquina') || c.toLowerCase().includes('máquina')); 
        if (colMaq) { 
            baseObj.forEach(fila => { 
                let m = fila[colMaq]; 
                if (m) { let mTrim = String(m).trim(); if (diccionarioLineas[mTrim]) fila["Línea de Producción"] = diccionarioLineas[mTrim]; else delete fila["Línea de Producción"]; } 
            }); 
        } 
    }); 
    guardarEstado('bd_industrial', baseDeDatos); guardarEstado('bd_costos', baseDeCostos); 
    document.getElementById('modalCategorizar').close(); actualizarSelectoresGlobales(); actualizarSelectoresCostos(); actualizarListasFiltrosSecundarios(); aplicarFiltro(); alert("¡Líneas asignadas!"); 
}
...
function ejecutarImportacionConMapeo() {
    ...
    if (Object.keys(diccionarioNombres).length > 0) { let columnas = Array.from(new Set(nuevosDatos.flatMap(Object.keys))); columnas.forEach(col => { nuevosDatos.forEach(fila => { let v = fila[col]; if (v) { let vTrim = String(v).trim(); if (diccionarioNombres[vTrim]) fila[col] = diccionarioNombres[vTrim]; } }); }); }
    if (Object.keys(diccionarioLineas).length > 0) { let columnas = Array.from(new Set(nuevosDatos.flatMap(Object.keys))); let colMaq = columnas.find(c => c.toLowerCase().includes('maquina') || c.toLowerCase().includes('máquina')); if (colMaq) { nuevosDatos.forEach(fila => { let maq = fila[colMaq]; if (maq) { let mTrim = String(maq).trim(); if (diccionarioLineas[mTrim]) fila["Línea de Producción"] = diccionarioLineas[mTrim]; } }); } }
...
function aplicarFiltro() {
    ...
    if (tipo === 'todo') { datosFiltrados = [...baseDeDatos]; } else { datosFiltrados = baseDeDatos.filter(f => { let vF = f[colFecMto]; if (!vF) return false; let vs = String(vF); if (tipo === 'dia') return vs === document.getElementById('inputDia').value; if (tipo === 'mes') return vs.startsWith(document.getElementById('inputMes').value); if (tipo === 'anio') return vs.startsWith(document.getElementById('inputAnio').value); if (tipo === 'mes_historico') return vs.substring(5, 7) === document.getElementById('inputMesHistorico').value; return false; }); }
    const colFecCosto = document.getElementById('costoColFecha').value;
    if (tipo === 'todo') { costosFiltrados = [...baseDeCostos]; } else { costosFiltrados = baseDeCostos.filter(f => { let vF = f[colFecCosto]; if (!vF) return false; let vs = String(vF); if (tipo === 'dia') return vs === document.getElementById('inputDia').value; if (tipo === 'mes') return vs.startsWith(document.getElementById('inputMes').value); if (tipo === 'anio') return vs.startsWith(document.getElementById('inputAnio').value); if (tipo === 'mes_historico') return vs.substring(5, 7) === document.getElementById('inputMesHistorico').value; return false; }); }
...
function renderizarBotoneraArbol() {
    ...
    if(!colL1 || baseObj.length === 0) { bot.innerHTML = '<span style="color:#999; font-size:12px;">Selecciona la columna "Línea" y quita filtros si la base está vacía.</span>'; document.getElementById('arbolTotalGlobal').innerText="0"; return; }
...
function renderizarJerarquia() {
    ...
    Object.keys(arbol).sort().forEach(maq => {
        ...
        Object.keys(arbol[maq]).sort().forEach(sec => {
            ...
            let tIdS = "tbl_" + Math.random().toString(36).substr(2,9);
            html += `<div class="caja-anidada nivel-seccion"><div class="caja-header" onclick="this.nextElementSibling.style.display = this.nextElementSibling.style.display==='none' ? 'block' : 'none'"><div class="tree-title-row"><span>🔧 Sección: <span style="color:#d97706;">${sec}</span></span><div><span style="font-weight:normal; font-size:12px; margin-right:10px;">(${filasSec.length} registros)</span> <span class="indicador-exp">🔽 Expandir Aplicaciones</span></div></div>${generarHtmlStats(filasSec, metrica)}</div><div class="caja-body" style="display:none;">`;
            html += `<button onclick="document.getElementById('${tIdS}').style.display = document.getElementById('${tIdS}').style.display === 'none' ? 'block' : 'none'" class="btn-chico" style="margin-bottom:10px;">👁️ Ver Datos Sección</button><div id="${tIdS}" style="display:none; overflow-x:auto; max-height:300px; margin-bottom:10px;">${generarMiniTabla(filasSec, metrica)}</div>`;

            Object.keys(arbol[maq][sec]).sort().forEach(app => {
                let filasApp = arbol[maq][sec][app]; let tIdA = "tbl_" + Math.random().toString(36).substr(2,9);
                html += `<div class="caja-anidada nivel-aplicacion"><div class="caja-header"><div class="tree-title-row"><span>🎯 Aplicación: <span style="color:#16a34a;">${app}</span></span><span style="font-weight:normal; font-size:12px;">(${filasApp.length} registros)</span></div>${generarHtmlStats(filasApp, metrica)}<button onclick="document.getElementById('${tIdA}').style.display = document.getElementById('${tIdA}').style.display === 'none' ? 'block' : 'none'" class="btn-chico" style="margin-top:5px;">👁️ Ver Datos App</button><div id="${tIdA}" style="display:none; overflow-x:auto; max-height:300px; margin-top:5px;">${generarMiniTabla(filasApp, metrica)}</div></div></div>`;
            }); html += `</div></div>`; 
        }); html += `</div></div>`; 
...
function generarHtmlStats(filas, metrica) {
    ...
    let agrupacion = document.getElementById('arbolAgrupacionCuartiles').value;
    
    if(metrica === 'frecuencia') {
        let colFecha = document.getElementById('selectorColumnaFecha').value; let pM = {};
        filas.forEach(f => { let d = f[colFecha]; if(d) { let fs = String(d); let key = ""; if(agrupacion==='dia') key=fs; else if(agrupacion==='mes') key=fs.substring(0,7); else if(agrupacion==='anio') key=fs.substring(0,4); else if(agrupacion==='semana') { let td = new Date(fs+"T12:00:00"); key = td.getFullYear()+"-"+Math.floor(td.getTime()/604800000); } if(key) pM[key] = (pM[key] || 0) + 1; } });
        dataArray = Object.values(pM).sort((a,b)=>a-b); if(dataArray.length===0) dataArray=[0]; suffix = " fallos/per"; sumaTotal = filas.length;
    }
    else if (metrica === 'costo') {
        let colCos = document.getElementById('arbolColCosto').value; let colFec = document.getElementById('costoColFecha').value; let tc = obtenerTC(); prefix = obtenerSimboloMoneda() + " "; let pC = {};
        filas.forEach(f => { let d = f[colFec]; let money = parsearMoneda(f[colCos])/tc; sumaTotal+=money; if(d) { let fs = String(d); let key = ""; if(agrupacion==='dia') key=fs; else if(agrupacion==='mes') key=fs.substring(0,7); else if(agrupacion==='anio') key=fs.substring(0,4); else if(agrupacion==='semana') { let td = new Date(fs+"T12:00:00"); key = td.getFullYear()+"-"+Math.floor(td.getTime()/604800000); } if(key) pC[key] = (pC[key] || 0) + money; } });
    ...
    if(metrica !== 'costo') {
        ...
        htmlStats += `<div class="tree-extras"><span>👨‍🔧 <b>Técnicos (Top 3):</b> ${topTec}</span><span>⏰ <b>Turnos:</b> ${topTur}</span></div>`;
    }
    return htmlStats;
}
...
function exportarCSVMantenimiento() { if (baseDeDatos.length === 0) return alert("Sin datos."); let expFilt = confirm("¿Exportar SOLO datos visibles (filtrados)?\n[Aceptar] = Filtrados\n[Cancelar] = Toda la base"); let dExp = expFilt ? datosFiltrados : baseDeDatos; if (dExp.length === 0) return alert("No hay datos."); let c = Array.from(new Set(dExp.flatMap(Object.keys))); descargarArchivo(Papa.unparse(dExp, { columns: c }), `mantenimiento_${expFilt?'filtrado':'completo'}.csv`, 'text/csv'); }
