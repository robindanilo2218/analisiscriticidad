...
function cambiarPestana(pestana) {
    ...
    if (pestana === 'jerarquia') { actualizarSelectoresArbol(); }
...
// MEJORADO: Cálculo ultra-preciso de horas para evitar números abismales. Convierte a Horas Decimales (Ej. 1.5 Hrs)
function calcularDiferenciaHoras(inicio, fin) { 
    function aMs(t) { 
        let s = String(t).toLowerCase().trim().replace(/[^0-9:apm]/g, ''); 
        if(!s) return null; 
        let isPM = s.includes('p'); let isAM = s.includes('a');
        let parts = s.replace(/[apm]/g, '').split(':'); 
        let h = parseInt(parts[0]) || 0; let m = parseInt(parts[1]) || 0;
        if(isPM && h < 12) h += 12; if(isAM && h === 12) h = 0;
        return (h * 60) + m; // Devuelve minutos desde medianoche
    } 
    let mI = aMs(inicio); let mF = aMs(fin);
    if(mI === null || mF === null) return 0;
    if(mF < mI) mF += (24 * 60); // Cruzó medianoche
    let diffMinutos = mF - mI;
    return parseFloat((diffMinutos / 60).toFixed(2)); // Devuelve Horas decimales
}
...
function generarVistaPrevia() { 
    const reglas = extraerReglasDeUI(); const head = document.getElementById('headPreview'); const body = document.getElementById('bodyPreview'); head.innerHTML = ''; body.innerHTML = ''; if (archivosTemporalesLeidos.length === 0) return; let destUnicos = Array.from(new Set(Object.values(reglas.mapaDestinos))); reglasCalculos.forEach(c => destUnicos.push(c.destino)); const colVisibles = Array.from(new Set(destUnicos)).sort(); if (colVisibles.length === 0) { body.innerHTML = '<tr><td>Todo ignorado.</td></tr>'; return; } 
...
function ejecutarImportacionConMapeo() {
    ...
    if (nuevosDatos.length === 0) return alert("No hay datos.");
...
function actualizarListasFiltrosSecundarios() {
    ...
    llenar('olapFiltroCol1', baseDeDatos); llenar('olapFiltroCol2', baseDeDatos); // Filtros del OLAP
}
...
// --- NUEVO: ÁRBOL JERÁRQUICO (CUBOS OLAP CON TOTALES Y FILTROS) ---
...
function renderizarBotoneraArbol() {
    ...
    // Aplicar Filtros Secundarios del OLAP a la base completa antes de sacar las líneas
    ...
    // CÁLCULO DEL TOTAL GLOBAL DEL OLAP
    let totalGlobal = 0; let suffix = "";
    if(metrica === 'frecuencia') { totalGlobal = baseObj.length; suffix = "Fallos"; }
    ...
    document.getElementById('arbolTotalGlobal').innerText = totalGlobal;
    document.getElementById('arbolSuffixGlobal').innerText = suffix;
...
function renderizarJerarquia() {
    ...
    // Aplicar los filtros de búsqueda y los secundarios del panel OLAP
    let busq = document.getElementById('busquedaOlap').value.toLowerCase().trim(); if (busq) baseObj = baseObj.filter(f => Object.values(f).some(v => String(v).toLowerCase().includes(busq)));
    let c1 = document.getElementById('olapFiltroCol1').value; let v1 = document.getElementById('olapFiltroVal1').value; let c2 = document.getElementById('olapFiltroCol2').value; let v2 = document.getElementById('olapFiltroVal2').value;
    if (c1 && v1) baseObj = baseObj.filter(f => String(f[c1]).trim() === v1); if (c2 && v2) baseObj = baseObj.filter(f => String(f[c2]).trim() === v2);
...
function generarHtmlStats(filas, metrica) {
    ...
    if(metrica === 'frecuencia') {
        ...
        filas.forEach(f => { let d = f[colFecha]; if(d) { let m = String(d).substring(0,7); pM[m] = (pM[m] || 0) + 1; } });
        dataArray = Object.values(pM).sort((a,b)=>a-b); if(dataArray.length===0) dataArray=[0]; suffix = " fallos/mes"; sumaTotal = filas.length;
    }
    else if (metrica === 'costo') {
        let colCos = document.getElementById('arbolColCosto').value; let tc = obtenerTC(); prefix = obtenerSimboloMoneda() + " ";
        filas.forEach(f => { let v = f[colCos]; if(v) { let money = parsearMoneda(v) / tc; dataArray.push(money); sumaTotal += money; } }); dataArray.sort((a,b)=>a-b);
    }
    else if (metrica === 'tiempo_muerto') {
        let colTM = document.getElementById('arbolColTM').value; isTiempo = true; suffix = " Hrs";
        filas.forEach(f => { let v = f[colTM]; if(v) { let nv = parseFloat(String(v).replace(/[^0-9\.]/g, '')); if(!isNaN(nv)) { dataArray.push(nv); sumaTotal += nv; } } }); dataArray.sort((a,b)=>a-b);
    }
    else if (metrica === 'tiempo_atencion') {
        let cHRep = document.getElementById('arbolColHRep').value; let cHIni = document.getElementById('arbolColHIni').value; isTiempo = true; suffix = " Hrs";
    ...
    let htmlStats = `<div class="tree-stats-row">
        <span class="stats-badge stats-badge-avg" style="background:#e0f2fe; border-color:#3b82f6; color:#1e40af;"><b>TOTAL:</b> ${prefix}${formatTotal(sumaTotal)}${metrica==='frecuencia'?' Fallos':suffix}</span>
        <span class="stats-badge stats-badge-q" title="Mínimo">Min: ${prefix}${formatNum(q0)}${suffix}</span>
        <span class="stats-badge stats-badge-q" title="Cuartil 1 (25%)">Q1: ${prefix}${formatNum(q1)}</span>
        <span class="stats-badge stats-badge-q" title="Mediana (50%)">Mediana: ${prefix}${formatNum(q2)}</span>
        <span class="stats-badge stats-badge-q" title="Cuartil 3 (75%)">Q3: ${prefix}${formatNum(q3)}</span>
        <span class="stats-badge stats-badge-q" title="Máximo">Max: ${prefix}${formatNum(q4)}</span>
        <span class="stats-badge stats-badge-avg" title="Promedio">Promedio: ${prefix}${formatNum(avg)}${suffix}</span>
    </div>`;
...
function sincronizarFrecYCostos() {
    const equipo = document.getElementById('critEquipoSelect').value; const periodo = document.getElementById('critPeriodo').value; const metodo = document.getElementById('critMetodoCalculo').value; const info = document.getElementById('critInfoAuto'); if(!equipo) { info.innerText = "Selecciona máquina."; return; }
    document.getElementById('critEquipo').value = equipo; const colMto = document.getElementById('critColMto').value; const colFecMto = document.getElementById('selectorColumnaFecha').value; let freq = 0; let mtoText = "0 fallos";
    if(colMto && colFecMto && baseDeDatos.length>0) {
        let fallos = baseDeDatos.filter(f => String(f[colMto]).trim() === equipo); let perU = {};
        fallos.forEach(f => { let fec = f[colFecMto]; if(fec) { let fs = String(fec); let pKey = ""; if(periodo === 'dia') pKey = fs; else if(periodo === 'mes') pKey = fs.substring(0,7); else if(periodo === 'anio') pKey = fs.substring(0,4); else if(periodo === 'semana') { let d = new Date(fs+"T12:00:00"); pKey = d.getFullYear()+"-"+Math.floor(d.getTime()/604800000); } if(pKey) perU[pKey] = (perU[pKey] || 0) + 1; } });
        let arrF = Object.values(perU).sort((a,b)=>a-b); if(arrF.length === 0) arrF = [0];
        if (metodo === 'promedio') freq = fallos.length / Math.max(1, arrF.length); else if (metodo === 'q0') freq = arrF[0]; else if (metodo === 'q1') freq = calcularPercentil(arrF, 25); else if (metodo === 'q2') freq = calcularPercentil(arrF, 50); else if (metodo === 'q3') freq = calcularPercentil(arrF, 75); else if (metodo === 'q4') freq = arrF[arrF.length-1];
        mtoText = `${freq.toFixed(1)}/per`; let sFrec = document.getElementById('critFrec'); if (freq <= 1.5) sFrec.value = "1"; else if (freq > 1.5 && freq <= 4.5) sFrec.value = "2"; else if (freq > 4.5 && freq <= 7.5) sFrec.value = "3"; else sFrec.value = "4";
    }
    const colCos = document.getElementById('critColBodega').value; const colValCos = document.getElementById('critColValorBodega').value; const colFecCos = document.getElementById('costoColFecha').value; let cost = 0; let cosText = "0"; let tc = obtenerTC(); let sim = obtenerSimboloMoneda();
    if(colCos && colValCos && colFecCos && baseDeCostos.length>0) {
        let registros = baseDeCostos.filter(f => String(f[colCos]).trim() === equipo); let perUC = {}; 
        registros.forEach(f => { let fec = f[colFecCos]; let valorDinero = (parsearMoneda(f[colValCos]) / tc); if(fec) { let fs = String(fec); let pKey = ""; if(periodo === 'dia') pKey = fs; else if(periodo === 'mes') pKey = fs.substring(0,7); else if(periodo === 'anio') pKey = fs.substring(0,4); else if(periodo === 'semana') { let d = new Date(fs+"T12:00:00"); pKey = d.getFullYear()+"-"+Math.floor(d.getTime()/604800000); } if(pKey) perUC[pKey] = (perUC[pKey] || 0) + valorDinero; } });
        let arrC = Object.values(perUC).sort((a,b)=>a-b); if(arrC.length === 0) arrC = [0];
        if (metodo === 'promedio') { let sumC = arrC.reduce((a,b)=>a+b,0); cost = sumC / Math.max(1, arrC.length); } else if (metodo === 'q0') cost = arrC[0]; else if (metodo === 'q1') cost = calcularPercentil(arrC, 25); else if (metodo === 'q2') cost = calcularPercentil(arrC, 50); else if (metodo === 'q3') cost = calcularPercentil(arrC, 75); else if (metodo === 'q4') cost = arrC[arrC.length-1];
        cosText = `${sim} ${cost.toFixed(0)}/per`; let sCosto = document.getElementById('critCosto'); if (cost <= 5000) sCosto.value = "1"; else if (cost > 5000 && cost <= 10000) sCosto.value = "2"; else sCosto.value = "3";
    }
    let lblM = document.getElementById('critMetodoCalculo').options[document.getElementById('critMetodoCalculo').selectedIndex].text; info.innerText = `Cálculo por ${periodo} (${lblM}): Frec: ${mtoText} | Gasto: ${cosText}.`; actualizarPreviewCriticidad();
}
